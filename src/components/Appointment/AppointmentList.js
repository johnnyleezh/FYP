...
function AppointmentList({ user }) {
  ...
  const rowDisplay = () => {
    if (list.length > 0) {
      ...
      return (
        <div style={{ fontSize: '1.5rem', textAlign: 'center', backgroundColor: '#FBE8A6' }}>
      ...
      <div className='apptContainer'>
        <>
          <div className='pageBtn'>
            {button && (
              <Button buttonStyle='btn--outline' onClick={() => setCreateOpen(true)}>
                Create
              </Button>
            )}
          </div>
        ...
        <div className='apptListHeader'>
          ...
          <div className='apptColumnTitleHeader'>Title</div>
        </div>
        {rowDisplay()}
